...
{
    defaultReload // overheat coming soon
};
#endregion
...
{
    public AudioClip firing, holster, unholster, reload, emptyMag;
}
[System.Serializable]
...
public class Weapon_SO : ScriptableObject
{
    ...
    private bool isMelee; // Internal use
    ...
    [Range(.01f,.99f)]public float cooledPercentage = .75f; READY FOR FUTURE UPDATE, OVERHEAT WEAPONS COMING SOON*/
    ...
    [Tooltip("FOV amount substracted from the current fov on shooting"), Range(0, 180)] public float FOVValueToSubstract;

    [Tooltip("FOV amount substracted from the current fov on shooting"), Range(0, 180)] public float AimingFOVValueToSubstract;
    ...
    [Tooltip("Attacking rythm")] public float attackRate;
...
public class Weapon_SOEditor : Editor
{
                ...
                case "Visuals":
                    switch (style)
                    {
                        ...
                        case 2:
                            ...
                            {
                                EditorGUI.indentLevel++;
                                EditorGUILayout.PropertyField(serializedObject.FindProperty("FOVValueToSubstract"));
                                EditorGUI.indentLevel--;
                            }
    ...
    private void WeaponStatsSharedVariables(Weapon_SO myScript)
    {
        ...
        else myScript.magazineSize = 100;
    ...
    private void WeaponVisualSharedVariables(Weapon_SO myScript)
    {
        ...
        if (myScript.applyFOVEffectOnShooting)
        {
            ...
            EditorGUI.indentLevel--;
            EditorGUILayout.PropertyField(serializedObject.FindProperty("AimingFOVValueToSubstract"));
            EditorGUI.indentLevel--;
        }
        EditorGUILayout.Space(5f);
